...
const CustomizedDot = (props: any) => {
  ...
  return (
    <g>
      ...
      <circle cx={cx} cy={cy} r={size} fill={color} fillOpacity={0.5} />
...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    const emotionColor =
      colors.find((c) => c.emotion === data.emotion)?.color || "#000";
    ...
    return (
      <div className="relative bg-white p-2 sm:p-3 border border-gray-200 rounded-md shadow-md text-xs sm:text-sm md:text-base">
        ...
        {/* Triangle pointer */}
        <div
          className="absolute w-0 h-0 left-1/2 -translate-x-1/2 bottom-[-8px] sm:bottom-[-10px] 
  ...
  return (
    <Card className="font-sans w-full h-full border-none">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart
          data={engagementHistory.slice(-8)}
